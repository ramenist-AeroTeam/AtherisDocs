  ...
  return (
    <aside className={`shrink-0 self-stretch transition-all ${open ? "w-56" : "w-10"} bg-card border-r flex flex-col relative z-10`}>
      <div className="flex items-center justify-between px-2 h-9 border-b">
        ...
        {open && (
          <button onClick={() => setShowCreate(true)} title="New tab" className="h-6 w-6 grid place-items-center rounded hover:bg-muted text-muted-foreground">
            <Plus className="h-3.5 w-3.5" />
          </button>
        )}
        <button onClick={() => setOpen((o) => !o)} className={`h-7 w-7 grid place-items-center rounded hover:bg-muted text-muted-foreground ${open ? "" : "ml-auto"}`} title={open ? "Collapse" : "Expand"}>
          ...
          return (
            ...
            >
              <div className="h-7 w-7 shrink-0 rounded-full overflow-hidden grid place-items-center text-[10px] font-bold relative"
                ...
                {prof?.avatar_url ? <img src={prof.avatar_url} alt="" className="h-full w-full object-cover" /> : initials(name)}
              </div>
              {open && (
              ...
              {open && isMe && (
                <span onClick={(e) => remove(t, e)} className="opacity-0 group-hover:opacity-100 h-6 w-6 grid place-items-center rounded hover:bg-destructive/15 hover:text-destructive transition">
      ...
      {showCreate && createPortal(
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/40" onClick={() => setShowCreate(false)}>
          <div className="bg-card border rounded-lg shadow-pop p-5 w-[360px] max-w-[92vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <input autoFocus value={newName} onChange={(e) => setNewName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && create()} placeholder="Tab name…" className="w-full h-9 px-3 rounded-md border bg-background text-sm mb-3" />
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-10 gap-1 max-h-32 overflow-y-auto p-1 border rounded-md bg-background">
                {EMOJI_PRESETS.map((em) => (
                  <button key={em} onClick={() => setNewEmoji(em)} className={`h-7 w-7 grid place-items-center rounded text-base hover:bg-muted transition ${newEmoji === em ? "bg-primary/15 ring-1 ring-primary" : ""}`}>
                    {em}
                  </button>
                ))}
              </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={create} disabled={busy || !newName.trim()} className="h-8 px-4 rounded-md text-sm bg-primary text-primary-foreground hover:opacity-90 disabled:opacity-50">
                Create
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-[100] grid place-items-center bg-black/40" onClick={() => setEmojiEditFor(null)}>
          <div className="bg-card border rounded-lg shadow-pop p-4 w-[320px]" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-10 gap-1 mb-3 max-h-48 overflow-y-auto">
              {EMOJI_PRESETS.map((em) => (
                <button key={em} onClick={() => setEmoji(emojiEditFor, em)} className="h-7 w-7 grid place-items-center rounded text-base hover:bg-muted transition">
                  {em}
                </button>
              ))}
            </div>
            <input placeholder="Or type any emoji and press Enter…" onKeyDown={(e) => {
              if (e.key === "Enter") { const v = (e.target as HTMLInputElement).value.trim(); if (v) setEmoji(emojiEditFor, v); }
